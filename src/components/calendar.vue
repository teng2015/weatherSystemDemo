<template>
  <div @click.stop=""  @touchstart.stop="" class="calendar" v-show="show" :style="{'left':x+'px','top':y+'px'}" transition="calendar" transition-mode="out-in">
    <div class="calendar-tools"> <i class="fa fa-angle-left float left" @click="prev"  @touchstart="prev"></i> <i class="fa fa-angle-right float right" @click="next"  @touchstart="next"></i>
      ...
      for (i = 1; i <= lastDateOfMonth; i++) {
        ...
        if (that.range) {
          ...
          if (that.rangeBegin.length > 0) {
            beginTime = Number(new Date(that.rangeBegin[0], that.rangeBegin[1], that.rangeBegin[2]))
            endTime = Number(new Date(that.rangeEnd[0], that.rangeEnd[1], that.rangeEnd[2]))
            thisTime = Number(new Date(that.year, that.month, i))
